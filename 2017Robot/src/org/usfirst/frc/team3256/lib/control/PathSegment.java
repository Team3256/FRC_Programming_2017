...
public class PathSegment {
    ...
    public static class Sample {
        ...
        public Sample(Translation translation, double speed) {
    ...
    protected Translation mStart;
    protected Translation mEnd;
    protected Translation mStartToEnd; // pre-computed for efficiency
    ...
    public static class ClosestPointReport {
        ...
        public Translation closest_point; // The result of
                                            // interpolate(clamped_index)
        public double distance; // The distance from closest_point to the query
                                // point
    }

    public PathSegment(Translation start, Translation end, double speed) {
        mEnd = end;
        mSpeed = speed;
        updateStart(start);
    }

    public void updateStart(Translation new_start) {
        mStart = new_start;
    ...
    // Index is on [0, 1]
    public Translation interpolate(double index) {
        return mStart.interpolate(mEnd, index);
    }

    public double dotProduct(Translation other) {
        Translation start_to_other = mStart.inverse().translateBy(other);
        return mStartToEnd.getX() * start_to_other.getX() + mStartToEnd.getY() * start_to_other.getY();
    }

    public ClosestPointReport getClosestPoint(Translation query_point) {
        ...
        if (mLength > kEpsilon) {
            ...
            rv.closest_point = new Translation(mStart);
        }
